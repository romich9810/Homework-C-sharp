        ...
        abstract class Unit
        {
            ...
            public void RecoveryHealthPoint() 
            {
                ...
                if (percentProbabiltyOfHealing > percentProbablityForFullHealing)
                {
                    ...
                    Console.WriteLine($"Боец готов к бою! HP - {HealthPoint}");
                }
                ...
                {
                    Console.WriteLine($"Бойца не удалось подлечить. Травмы несоизмеримы с жизнью. Нынешнее здоровье  - {HealthPoint}");
                }
            }
        }
        ...
        class Solder: Unit
        {
            ...
            public override void TakeDamage(int damage)
            {
                ...
                if (IsAlive)
                {
                    ...
                    {
                        HealthPoint -= damage + ((damage - (damage * Armor / fullPercent)) * Morale / fullPercent);
                    }
                    else
                    {
                        HealthPoint -= damage - ((damage - (damage * Armor / fullPercent)) * Morale / fullPercent);
        ...
        class Squad
        {
            ...
            public void ShowCombatReady()
            {
                foreach(Unit unit in _solders)
                {
                    if (unit.IsAlive)
                    {
                        Console.Write($"{unit.Role} - {unit.HealthPoint} HP, {unit.Damage} урона");

                        if (unit is SubmachineGunner submachineGunner)
                        {
                            Console.Write($", {submachineGunner.Equipment} гранат");
                        }

                        if (unit is Medic medic)
                        {
                            Console.Write($", {medic.Equipment} аптечек");
                        }
            ...
            public bool IsCommandAlive()
            {
                ...
                Comander comander = new Comander(random);
            ...
            public void Attack(Squad enemy)
            {
                ...
                SubmachineGunner submachineGunner = new SubmachineGunner(random); 
                ...
                for (int i = 0; i < countFlowGrenates; i++)
                {
                    ...
                    while (attackingSolder.Role != submachineGunner.Role);
            ...
            {              
                ...
                Medic medic = new Medic(random);

                List<Unit> potentialWounded = GiveRandomSolders(random);

                int minCountMedics = 1;
                int maxCountMedics = CalculateUnits(medic);
                ...
                {
                    TryFindUnit(medic, out Unit foundedMedic);

                    if (foundedMedic.IsAlive)
                    {
                        Console.WriteLine($"{foundedMedic.Role} осмотрел {potentialWounded.Count} бойцов:");

                        foreach (Unit unit in potentialWounded)
                        {
                            ((Medic)foundedMedic).Heal(unit);
                        }
                    }
                    else
                    {
                        Console.WriteLine($"{foundedMedic.Role} погиб и не может оказать помощь.");
                    }
                }
            }
            ...
            private int CalculateUnits(Unit unitCalculate)
            {
                int count = 0;

                foreach(Unit unit in _solders)
                {
                    if (unit.Role == unitCalculate.Role)
                    {
                        count++;
                    }
                }
            ...
            private bool TryFindUnit(Unit solder, out Unit foundSolder)
            {
                foundSolder = null;
        ...
        class Battlefield
        {
            ...
            public void Fight()
            {
                ...
                while (_firstArmy.IsCapableFight() & _secondArmy.IsCapableFight())
                {
                    ...
                    if (stage == fistStage)
                    {
                        Console.WriteLine($"Ход стороны {_firstArmy.Side}\n") ;

                        _firstArmy.IsComanderComand();
                        _firstArmy.Attack(_secondArmy);
                        _firstArmy.BoostMorale();
                        _firstArmy.ProvideMedicalAssistance();
                    }
                    else
                    {
                        Console.WriteLine($"Ход стороны {_secondArmy.Side}\n");

                        _secondArmy.IsComanderComand();
                        _secondArmy.Attack(_firstArmy);
                        _secondArmy.BoostMorale();
                        _secondArmy.ProvideMedicalAssistance();
                    }
